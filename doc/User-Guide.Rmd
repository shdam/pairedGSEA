...
`pariedGSEA` is an R package that allows you to run a paired differential gene
expression and splicing analysis.
Providing a count matrix, `pariedGSEA` combines the results of
`DESeq2` and `DEXSeq`, aggregates the p-values to gene level and allows
...
the FDR-adjusted p-values (see `?paired:::aggregate_pvalue`
...
level results found in the `results` folder under the names "\*_deseqres.RDS"
and "\*_dexseqres.RDS" for the DGE and DGS analysis, respectively
...
There are a range of other parameters you can play with to tailor-make
...
1. You can use additional columns in your metadata as covariates in the
model matrix by setting `covariates` to a character vector of the
specific names. This will be used in SVA, DGE, and DGS.
2. The test should be kept at default settings, but advanced users may use
the "wald" test instead, if they wish.
3. The `...` parameters will be fed to `DESeq2::DESeq`,
see their manual for options.
4. If you want a stricter, more loose, or more advanced pre-filtering
...
`pairedGSEA` comes with a wrapper function for `fgsea::fora`. If you wish,
...
For this purpose, feel free to use the internal `msigdbr` wrapper function in
...
significant pathways using `plotly`. You can color specific points

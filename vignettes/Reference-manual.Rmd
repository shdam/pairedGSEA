...
date: "Last updated on May 28, 2022"
vignette: >
...
But before running the `paired_de` function, we recommend storing the experiment parameters in a set of variables at the top of your script for future reference and easy access:
...
experiment_title <- "Investigating healthy vs gene knockout" # Name of your experiment. This is used in the file names that are stored if store_results is TRUE (recommended)

```

### Running the analysis

The paired DGE/DGS analysis is the run with `paired_de`.
`paired_de` is essentially a wrapper function around `DESeq2::DESeq` and `DEXSeq::DEXSeq`, the latter takes in the ball park of 20-30 minutes to run.
...
After running the analyses, `paired_de` will aggregate the p-values to gene level using lancaster aggregation (see `?paired:::aggregate_pvalue` for more information) and calculate the FDR-adjusted p-values.
...
There are a range of other parameters you can play with to tailor-make the experience. Here, the default settings are showed. See `?pairedGSEA::paired_de` for further details.

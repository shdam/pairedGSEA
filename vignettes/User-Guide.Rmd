...
Welcome to the User Guide for `pariedGSEA`!
`pariedGSEA` is an R package that allows you to run a paired differential gene expression and splicing analysis.
Providing a count matrix, `pariedGSEA` combines the results of `DESeq2` and `DEXSeq`, aggregates the p-values to gene level and allows you to run a subsequent gene set over-representation analysis using its implementation of the `fgsea::fora` function. 
...
This data may also be in the format of a SummarizedExperiment or DESeqDataSet.
Please ensure that the rownames have the format: `gene:transcript`.
...
## Load data
data(example_se) # In this vignette we will use the included example Summarized Experiment. See ?example_se for more information about the data.
...
group_col <- "group_nr" # Name of column that specifies the groups you would like to compare
...
baseline <- 1 # Name of baseline group, as they are named in the group_col column
case <- 2 # Name of case group, as they are named in the group_col column
experiment_title <- "TGFb treatment for 12h" # Name of your experiment. This is used in the file names that are stored if store_results is TRUE (recommended)
...
all(colnames(example_se) %in% SummarizedExperiment::colData(example_se)[[sample_col]])
...
`paired_diff()` is essentially a wrapper function around `DESeq2::DESeq` [@deseq2] and `DEXSeq::DEXSeq` [@dexseq], the latter takes in the ballpark of 20-30 minutes to run depending on the size of the data and computational resources. Please visit their individual vignettes for further information.
...
diff_results <- paired_diff(
  object = example_se,
  metadata = NULL, # Use with count matrix or if you want to change it in the input object
  group_col = group_col,
  sample_col = sample_col,
  baseline = baseline,
  case = case,
  experiment_title = experiment_title,
  store_results = FALSE # Set to TRUE (recommended) if you want to store intermediate results, such as the results on transcript level 
  )

diff_results
```

After running the analyses, `paired_diff` will aggregate the p-values to gene level using lancaster aggregation [@lancaster] and calculate the FDR-adjusted p-values (see `?paired:::aggregate_pvalue` for more information).
...
If you set `store_results = TRUE`, you could extract the transcript level results found in the `results` folder under the names "\*_deseqres.RDS" and "\*_dexseqres.RDS" for the DGE and DGS analysis, respectively (The \* corresponds to the provided experiment title).
...
1. You can use additional columns in your metadata as covariates in the model matrix by setting `covariates` to a character vector of the specific names. This will be used in SVA, DGE, and DGS.
2. The test should be kept at default settings, but advanced users may use the "wald" test instead, if they wish.
3. The `...` parameters will be fed to `DESeq2::DESeq`, see their manual for options.
4. If you want a stricter, more loose, or more advanced pre-filtering (generally not necessary, as DESeq2 and DEXSeq performs their own pre-filtering), you can set the parameter to a different value or NULL and use your own pre-filtering method directly on the counts matrix.
...
`pairedGSEA` comes with a wrapper function for `fgsea::fora`. If you wish, feel free to use that directly or any other gene set analysis method - investigate the `diff_results` object before use to see what information it contains.
...
Before you get going, you will need a list of gene sets (aka. pathways) according to the species you are working with and the category of gene sets of interest.
...
ora <- paired_ora(
  ...
  gene_sets = gene_sets,
  experiment_title = NULL # experiment_title - if not NULL, the results will be stored in an RDS object in the 'results' folder
  )
...
The function allows you to interactively look at the placement of the significant pathways using `plotly`. You can color specific points based on a regular expression for gene sets of interest.
